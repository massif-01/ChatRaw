...
Agent Skills are reusable instruction packages stored as `SKILL.md` files. In ChatRaw v1, skills are
...
The catalog stores enablement, trust metadata, source information, timestamps, and diagnostics.
...
GitHub install is intentionally conservative. Ambiguous tree URLs, repository roots, private repos, path
traversal, multiple `SKILL.md` files, missing `name`, missing `description`, invalid names, oversize files,
and disallowed package paths are rejected by the backend.
...
Type `/` at the beginning of a token and choose a skill suggestion. ChatRaw adds an active skill chip and
removes the slash token from the visible message. The chip applies only to the next send and is cleared
...
- ChatRaw v1 never executes files in `scripts/`.
- `references/` and `assets/` are not read into chat automatically; only safe paths are summarized.
- `allowed-tools` is parsed as unsupported v1 frontmatter and does not grant permissions.
- `trusted` is governance metadata for future implicit matching. It does not bypass Skill Manager,
  disabled-skill checks, or explicit `/skill-name` activation.
- No implicit skill matching is performed in v1.

### Troubleshooting

- **No slash suggestions**: confirm Skill Manager is installed and enabled, then refresh the skills list.
- **Skill is not injected**: confirm the skill is enabled and the active chip is present before sending.
- **Install rejected**: check `name`, `description`, file size, package paths, and GitHub URL specificity.
- **Duplicate skill**: enable overwrite in Skill Manager and retry.
- **GitHub tree is ambiguous**: use a raw/blob `SKILL.md` URL or a more specific directory URL.
- **HTML appears in preview**: preview is plain text; HTML-like content is not rendered.

## 中文

Agent Skills 是以 `SKILL.md` 保存的可复用指令包。在 ChatRaw v1 中，skills 安装到本地，
...
Catalog 保存启用状态、trust 元数据、来源、时间戳和诊断信息。真正的指令正文仍以
`SKILL.md` 为准。
...
GitHub 安装是保守的。模糊 tree URL、仓库根目录、私有仓库、路径穿越、多个 `SKILL.md`、
缺少 `name`、缺少 `description`、非法名称、超大文件和不允许的包路径都会被后端拒绝。
...
在 token 开头输入 `/` 并选择 skill 建议。ChatRaw 会添加 active skill chip，并从可见输入中删除 slash token。
chip 只对下一次发送生效，发送成功后会清空。
...
- ChatRaw v1 永远不会执行 `scripts/` 中的文件。
- `references/` 和 `assets/` 不会自动读入聊天，只会摘要安全路径。
- `allowed-tools` 在 v1 中会作为不支持的 frontmatter 处理，不会授予权限。
- `trusted` 是为未来隐式匹配准备的治理元数据，不会绕过 Skill 管理器、禁用状态检查或显式 `/skill-name` 调用。
- v1 不做隐式 skill 匹配。

### 排错

- **没有 slash 建议**：确认 Skill 管理器已安装并启用，然后刷新 skills 列表。
- **Skill 没有注入**：确认该 skill 已启用，并且发送前存在 active chip。
- **安装被拒绝**：检查 `name`、`description`、文件大小、包路径和 GitHub URL 是否足够具体。
- **重复 skill**：在 Skill 管理器中启用 overwrite 后重试。
- **GitHub tree 模糊**：改用 raw/blob `SKILL.md` URL，或使用更具体的目录 URL。
